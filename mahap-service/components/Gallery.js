...
const galleryItems = [
  { src: campaignImages.hero, alt: 'Ambiente corporativo limpo pela Mahap Service', span: 'col-span-1 sm:col-span-2 sm:row-span-2' },
...
export default function Gallery() {
  return (
    <section id="galeria" aria-labelledby="gallery-heading" className="py-16 sm:py-24 lg:py-32">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {galleryItems.map((item, idx) => (
            <ScrollReveal key={idx} variant="scaleIn" delay={idx * 0.07} className={item.span}>
              <RevealImage className="relative aspect-[4/3] sm:aspect-square group h-full rounded-2xl bg-[var(--bg-subtle)]">
                <CampaignImage
                  src={item.src}
                  alt={item.alt}
                  fill
